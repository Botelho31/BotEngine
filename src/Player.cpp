...
void Player::YMovement(float dt){
    ...
    if((!physics->IsGrounded()) && (speed.y > 0) && (physics->DistanceTo(physics->GetCollider()->box,0,1,21) > 10) && (falling == false) && (!hittheground->Started()) && (!jumpanimation->Started()) && (!damagetimer->Started()) && (!swordattack->Started())){
        SetSprite(spritefiles["falling"],8,0.04);
        falling = true;
    }
    if(walldrag && !(physics->IsRight(1,false)) && !(physics->IsLeft(1,false)) && (!physics->IsGrounded()) && (speed.y > 0) && (physics->DistanceTo(physics->GetCollider()->box,0,1,21) > 10) && (!hittheground->Started()) && (!jumpanimation->Started()) && (!damagetimer->Started()) && (!swordattack->Started())){
        SetSprite(spritefiles["falling"],8,0.04);
        falling = true;
    }
...
void Player::KillPlayer(){
    ...
    Sprite *deadsprite = new Sprite(*deadObj,spritefiles["dead"],28,0.06,0,false);
    int xoffset = -40;
    if(playersprite->IsFlipped()){
        deadsprite->Flip();
        xoffset = 40;
